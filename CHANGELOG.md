...
- **3 个高价值 Code Agent 工具加入 eca-common-tools**
  - `test_result_summarize` — 解析测试输出，自动识别 pytest/jest/go test 框架，只返回失败用例与错误摘要（默认 top 10，含文件:行:列），解决验证反馈回路慢痛点
  - `stack_trace_analyze` — 多语言堆栈解析（Python/JS/Java/Go 启发式识别），输出错误类型、调用链摘要与首个项目内文件位置，解决错误定位难痛点
  - `code_block_extract` — 从 LLM 输出/文档提取代码块，支持语言过滤与序号选取，检测未闭合代码块（``` 奇数）输出截断预警，解决 LLM 输出不可靠痛点
  - 工具总数 7 → 10，全部纯 JS 零依赖
