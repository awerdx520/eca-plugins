...
## 2026-08-02 — 新增 3 个高价值 Code Agent 工具验证要点与启发式解析边界

**决策**：eca-common-tools 新增 test_result_summarize（pytest/jest/go 测试输出摘要）、stack_trace_analyze（Python/JS/Java/Go 堆栈解析）、code_block_extract（代码块提取），工具总数 7→10，全部纯 JS 零依赖。实现采用「框架/语言特征自动探测 + 启发式正则解析」，不引入 AST 或第三方库。

**理由**：
- 验证反馈回路慢（P2）与错误定位难（P3）是 Code Agent 最高频隐性成本，测试输出几百行只有几行失败信息、堆栈几十帧只有首个项目内位置有用——启发式解析足够满足"提取关键信息"需求，无需 AST 级精度
- 冒烟测试发现并修复 2 个真实格式陷阱：① pytest 成功输出（`3 passed`）无失败特征词，detectTestFramework 需追加成功特征；② 标准 V8 堆栈函数名小写（`at render`）被"排除 Java"防御检查误伤，Java 帧（`Main.java:5` 无列号）已被 `:\d+:\d+` 形式天然区分，防御检查冗余应删
- 经验：启发式解析工具的测试样例必须用真实输出格式（含缩进、贪婪匹配陷阱），伪造样例会掩盖格式边界问题
